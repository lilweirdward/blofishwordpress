	...
	$.wc_square_admin = {
		...
		sync: function( process, type ) {
			...
			$.post( wc_square_local.ajaxurl, data, function( response ) {
				...
				if ( 'done' === response.process ) {
					...
					$( '.wc-square-progress-bar span' ).animate( { width: response.percentage + '%' }, 30, 'swing', function() { 
						$( '.wc-square-progress-bar' ).fadeOut( 'slow', function() { 
							alert( response.message );
						});
					});

				} else {
					$( '.wc-square-progress-bar span' ).animate( { width: response.percentage + '%' }, 30, 'swing' );
		...
		init: function() {
			...
			$( '.woocommerce_page_wc-settings' ).on( 'click', '#wc-to-square, #square-to-wc', function( e ) {
				...
				$( '.wc-square-progress-bar span' ).animate( { width: '1%' }, 30, 'swing' );
