...
class WC_Square_Bulk_Sync_Handler {
	...
	public function wc_to_square_ajax() {
		...
		set_transient( 'sq_wc_sync_current_process', 'wc_to_square', apply_filters( 'woocommerce_square_syncing_current_process_cache_time', $cache_age ) );
		...
		if ( ( 0 === $process ) && $sync_products ) {
			...
			// cache it
			set_transient( 'wc_square_syncing_square_product_ids', $square_product_ids, apply_filters( 'woocommerce_square_syncing_square_ids_cache', $cache_age ) );
	...
	public function get_all_product_ids() {
		...
		foreach ( $products_with_skus as $product_id => $parent_id ) {
			...
			$product_ids[] = $parent_id ? $parent_id : $product_id;
	...
	public function delete_all_caches() {
		...
		delete_transient( 'wc_square_syncing_wc_product_ids' );

		delete_transient( 'wc_square_syncing_square_product_ids' );
